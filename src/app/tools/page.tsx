      ...
      <main className="flex-1 w-full positivus-container py-16">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-4">
            Browse All <span className="text-[#b9ff66]">Conversion</span> Tools
          </h1>
        ...
        <div className="flex flex-col gap-6 mb-12">
          ...
          <div className="relative w-full max-w-xl mx-auto">
            ...
            <input
              ...
              className="w-full pl-12 pr-5 py-4 rounded-[14px] bg-card border border-border text-foreground placeholder-muted-foreground focus:outline-none focus:border-[#b9ff66] transition-all text-sm"
            />
            {searchQuery && (
              <button 
                onClick={() => setSearchQuery("")}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-muted-foreground hover:text-foreground touch-target"
              >
                Clear
              </button>
            )}
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold rounded-full border transition-all whitespace-nowrap cursor-pointer ${
                    isActive
        ...
        <motion.div 
          layout 
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[40px] max-xl:gap-[30px]"
        >
            ...
            {filteredTools.map((tool) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative flex flex-col justify-between p-8 rounded-[30px] border cursor-pointer transition-all duration-200 select-none overflow-hidden ${
                    tool.category === 'pdf' || tool.category === 'image'
                      ? 'positivus-card-green text-[#191a23]' 
                      : tool.category === 'spreadsheet' || tool.category === 'presentation'
                      ? 'positivus-card-dark' 
                      : 'positivus-card'
                  }`}
                >
        ...
        {filteredTools.length === 0 && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-20 positivus-card max-w-md mx-auto"
          >
            <Compass size={48} className="text-muted-foreground mx-auto mb-4 animate-pulse" />
            <h3 className="text-lg font-bold text-foreground mb-2">No matching tools found</h3>
            ...
            <button
              ...
              className="mt-6 px-6 py-3 text-sm font-bold positivus-btn-primary cursor-pointer"
            >
              Reset Filters
            </button>
          </motion.div>
